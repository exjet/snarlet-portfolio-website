...
const projects = [
  ...
  },
  {
    ...
    images: [
      ...
      "assets/images/project-page/Scene 3.jpg",
...
const showCards = (filter = 'all') => {
  ...
  // Apply filter
  if (filter === 'animated') {
    filteredProjects = projects.filter(project => project.isAnimated === true);
  } else if (filter === 'non-animated') {
    filteredProjects = projects.filter(project => project.isAnimated === false);
  }

  filteredProjects.forEach(({ title, cardImage, images, Previewlink, Githublink }, index) => {
    // Find the original index in the projects array for proper gallery opening
    const originalIndex = projects.findIndex(p => p.title === title);
    output += `
        <div class="column skill-card card" style="margin: 15px"data-aos="zoom-in-up" data-aos-easing="linear" data-aos-delay="300" data-aos-duration="600" >
          <div class="wrapper" style="background: url('${cardImage}') center / cover no-repeat; cursor: pointer;" onclick="openImageGallery(${originalIndex});">
            <div class="header">
            </div>
            <div class="data">
              <div class="content">
              <div class="title-div">
                <h1 class="title"><a href="#" onclick="return false;">${title}</a></h1>
                </div>
            <ul class="menu-content"><br>
                  <li><a href="#" onclick="event.stopPropagation(); openImageGallery(${originalIndex}); return false;" class="social-icon"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="25" viewBox="0 0 30 28" fill="none" stroke="#fff" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-monitor"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg></a></li>
...
function showImage(index) {
  ...
  document.getElementById('prevBtn').style.display = currentProjectImages.length > 1 ? 'block' : 'none';
  document.getElementById('nextBtn').style.display = currentProjectImages.length > 1 ? 'block' : 'none';
}
